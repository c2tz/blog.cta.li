...
test("uses the Material Web Konachan menu with keyboard selection", async ({ page }) => {
  await gotoRoute(page, "/");
  ...
  const menu = page.locator("md-menu.home-anime-rating-menu");
  const safeItem = page.locator('md-menu-item[data-konachan-rating-option="safe"]');
  const questionableItem = page.locator('md-menu-item[data-konachan-rating-option="questionable"]');

  await expect(menu).toHaveAttribute("positioning", "popover");
  await expect(menu).not.toHaveAttribute("quick", "");
  await expect
    .poll(() =>
  ...
  await expect
    .poll(() => safeItem.evaluate((element) => element.matches(":focus-within")))
    .toBe(true);
...
test("keeps every search sort option visible above the dialog surface", async ({ page }) => {
  ...
  const expectOptionFocus = async (option: typeof relevanceOption) => {
    ...
    await expect
      ...
      .toEqual({ running: false, width: "3px" });
  };
  ...
  expect(sortVisualState.chevronColor).not.toBe(sortVisualState.focusColor);
